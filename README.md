...
```
  ...
  ──────────────►        │       ↓              ↓                  │
                         ...
                         │       ↓                                  │
                         │  GraphLinker → quality gate              │
                         │       ↓ (publish if trustworthy)        │
                         └────────────────┬────────────────────────┘
                                          │ data/journeys.json
                                          │ data/audio/{id}/*.wav
                                          ▼
...
**Key design principle:** the web runtime needs no Gemini API key. All content and audio is pre-generated by the pipeline and served as static files. The only runtime AI is the MiniLM embedding model for semantic search — local, no network, no cost.
